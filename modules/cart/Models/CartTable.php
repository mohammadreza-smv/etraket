...
class CartTable
{
    ...
    public function add_session_cart_to_table($user,$cart=[]){
        $this->user=$user;
        if(sizeof($cart)==0){
    ...
    public function get_cart_data(){
        $user_id=Auth::user()!=null ? Auth::user()->id : Auth::guard('api')->user()->id;
        $data=[];
    ...
    public function add($priceVariation){
        $this->user=(Auth::user()) ? Auth::user() : Auth::guard('api')->user();
