...
class Category extends CustomModel
{
    ...
    public static function getCatIdWithChild($catId){
        $catId=[$catId=>$catId];
        $catList=self::where('id',$catId)->with('getChild.getChild.getChild')->first();
        if($catList){
            foreach ($catList->getChild as $key1=>$value1){
                $catId[$value1->id]=$value1->id;
                foreach ($value1->getChild as $key2=>$value2){
                    $catId[$value2->id]=$value2->id;
                    foreach ($value2->getChild as $key3=>$value3){
                        $catId[$value3->id]=$value3->id;
                    }
                }
            }
        }
