<template>
    <div>
        ...
        <component v-else :is="componentName"  :key="n"/>
    </div>
</template>
    ...
    export default {
        ...
        methods:{
                ...
                try{
                    ...
                    if(response.status===200){
                        ...
                        setTimeout(function () {
                            ...
                            self.componentName='view-component';
                            self.n++;
                    ...
                    if(e.response.status===302){
                        window.location.href=e.response.data['reload'];
                    }
                }
            ...
            send_post_request:function (url,data,headers) {
                ...
                this.axios.post(url,data,headers).then(async response => {
                    ...
                    if (response.data['content'] !== undefined) {
                        const template = response.data.content;
                        const self = this;
                        if (response.data.js_tags !== undefined) {
                            await this.addJsFile(response.data.js_tags);
                        }
                        setTimeout(function () {
                            ...
                            self.componentName = 'component' + this.n;
                            self.n++;
                            self.$nextTick(function () {
                                const tags = document.getElementsByClassName('router-link');
                                self.addClickEvent(tags);
                    ...
                    this.$root.$emit('hide_progress',null);
                });
            },
        }
    }
